...
/**
 * A modified variant of the TeleopStructure.java file used in previous years
 * (original author: Lizzy Milford)
 * 
...
public class TeleopControl {
    ...
    // The ID used to designate which method is currently being used, keeps track of
    // button values
    private int oncePerPressMethodID = 0;
    private int toggleMethodID = 0;

    // List to hold values for each runOncePerPress instance, effectively keeping
    // track of all buttons
    ...
    public void runOncePerPress(boolean button, Runnable action) {
        ...
        // Init all the button statues, simply add it to the list in the order they
        // appear in Robot.java
        if (firstLoop) {
        ...
        if (button) {
            ...
            // Check that the current button has NOT been pressed already, if so run the
            // function
            if (pressedStatusList.get(oncePerPressMethodID) == false) {
                action.run();
            }

            // Now that it has been run once change the value in the list to true signifying
    ...
    /**
     * Used to toggle running a function on or off, called from the overhead method
     * toggle which utilizes the run once per press method
     * 
    ...
    /**
     * Called to tell the toggling method to actually toggle the method, because its
     * easier to use a run once per press than not
     * 
